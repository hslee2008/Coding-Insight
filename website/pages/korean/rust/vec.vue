<template>
  <div>
    <h2>Vec</h2>
    <p>
      Vec는 배열인데 매일 값을 넣거나 제거할 때마다 배열의 크기가 변합니다.<br /><br />

      일반 배열은 항상 크기가 정해져 있기 때문에 값을 제거하거나 새롭게 넣을 수
      없습니다.<br /><br />

      Vec는 <code v-highlight class="rust ic">Vec&lt;데이터></code>로 선언할 수
      있습니다.
    </p>

    <Header type="Rust" title="백터" num="41" />

    <h2>Vec 사용하기</h2>
    <p>
      vec!라는 매크로를 사용하면 다음에 나오는 배열은 바로 Vec로 선언할 수
      있습니다.<br /><br />

      <code v-highlight class="rust ic">vec![1, 2, 3, 4, 5]</code>

      Vec는 크기가 지정되지 않았기 때문에
      <code v-highlight class="rust ic">.push()</code> 메서드나
      <code v-highlight class="rust ic">.pop()</code>를 사용해 새로운 값을
      <mark>끝</mark>에 추가하거나 <mark>끝</mark>의 값을 제거할 수 있습니다.<br /><br />

      참고로 Vec에 새로운 값을 추가하거나 삭제하기 위해서는 mut 변수여야
      합니다.<br /><br />

      그리고 Vec를 출력하기 위해서는 배열과 같이
    ...
    <h2>len() 메서드와 인덱스</h2>
    <p>
      <code v-highlight class="rust ic">len()</code> 메서드를 사용하면 배열의
      크기를 알 수 있습니다.<br /><br />

      그리고 배열처럼 []를 사용해 사이에 숫자를 넣으면 그 인덱스에 있는 값을 알
      수 있습니다.<br /><br />

      그리고 <code v-highlight class="rust ic">iter()</code>르 사용하면 각
      아이템을 순서대로 순회할 수 있습니다.<br /><br />

      마지막으로 iter할 때 값을 바꾸고 싶다면 항상
      <code v-highlight class="rust ic">iter_mut()</code>를 사용하고 반환된 값
    ...
    <h2>enumerate() 메서드</h2>
    <p>
      <code v-highlight class="rust ic">enumerate()</code> 메서드를 사용하면 각
      아이템을의 인덱스와 값이 함께 순회할 수 있습니다.<br /><br />

      (인덱스, 값) 튜플을 순회하면서 인덱스를 출력하고 값을 출력합니다.<br /><br />

      <code v-highlight class="rust">
        for (index, value) in veccy.iter().enumerate() {<br />
        &nbsp;&nbsp;println!("{}에 {}", value, index);<br />
        }
      </code>

      그리고 <code v-highlight class="rust ic">enumerate()</code>를 사용하면
