<template>
  <div>
    <OnlyTextHeading
      heading="char 데이터 형태"
      :contents="[
        'char은 한 알파벳을 저장하는 데이터 형태입니다.',
        '사실 다른 코딩 언어는 문자를 저장하는데 1 BYTE (8bit)를 사용하는데, Rust는 사실 4 BYTE를 이용합니다.',
    ...
    <BasicExpDivWithOneIframe
      heading="char 데이터 형태 선언"
      :contents="[
        'char은 \'\' 안에 한 문자를 넣어서 선언할 수 있습니다.',
    ...
    <BasicExpDivWithOneIframe
      heading="char 값에 대한 정보 얻기"
      :contents="[
        'is_alphabetic() 메서드는 char 값이 알파벳인지 확인합니다.',
        'is_lowercase() 메서드는 char 값이 소문자인지 확인합니다.',
        'is_uppercase() 메서드는 char 값이 대문자인지 확인합니다.',
        'is_whitespace() 메서드는 char 값이 공백인지 확인합니다.',
        'is_alphanumeric() 메서드는 char 값이 알파벳 또는 숫자인지 확인합니다.',
        'is_numeric() 메서드는 char 값이 숫자인지 확인합니다.',
        'is_ascii() 메서드는 char 값이 ASCII 문자인지 확인합니다.',
      ]"
    ...
    <BasicExpDivWithOneIframe
      ...
      iframe="https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&code=fn%20main()%20%7B%0D%0A%20%20%20%20let%20val1%3A%20char%20%3D%20%27a%27%3B%0D%0A%20%20%20%20%0D%0A%20%20%20%20println!(%22%7B%7D%22%2C%20val1.to_uppercase())%3B%0D%0A%20%20%20%20println!(%22%7B%7D%22%2C%20val1.to_lowercase())%3B%0D%0A%7D"
    />
